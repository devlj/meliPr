...
STAGE_NAME="development"  # Nombre del Stage en API Gateway

# Función para desplegar el API en API Gateway
deploy_api_gateway() {
    ...
    echo "🚀 **Desplegando cambios en API Gateway en el stage $STAGE_NAME...**"
    aws apigateway create-deployment --rest-api-id "$API_ID" --stage-name "$STAGE_NAME" --region "$REGION"
    echo "✅ Despliegue exitoso en el stage '$STAGE_NAME'."
}


# Función para empaquetar la Lambda con `lambda_function.py`
package_lambda() {
...
# Función para crear/actualizar Lambda con arquitectura x86_64
deploy_lambda() {
    package_lambda
    ...
    if [ -z "$LAMBDA_ARN" ]; then
        echo "🚀 Creando función Lambda $LAMBDA_NAME con arquitectura x86_64..."
        ...
        echo "✅ Lambda creada con arquitectura x86_64 y ARN: $LAMBDA_ARN"
    else
